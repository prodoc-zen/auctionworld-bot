...
async def get_weekly_seconds(session, discord_id, start, now):
...
def register(tree, database):
    ...
    async def weeklyhours(interaction, member: discord.Member = None):
        ...
        quota_hours = int(interaction.client.config.get("weekly_quota_hours", DEFAULT_WEEKLY_QUOTA_HOURS))
            ...
            if member is None:
                ...
                if not admins:
                    await interaction.response.send_message(
                        "No active admin users are registered yet.",
                        ephemeral=True,
                    )
                    return
                ...
                for admin in admins:
                    ...
                    active_text = " active" if active_count else ""
                    lines.append(
                        f"<@{admin.discord_id}> - {format_duration(total_seconds)}{active_text}"
                    )
        ...
        quota_seconds = quota_hours * 60 * 60
        remaining_seconds = max(0, quota_seconds - total_seconds)
        percent = 100 if quota_seconds == 0 else min(100, round((total_seconds / quota_seconds) * 100))
        ...
        embed.add_field(name="Remaining", value=format_duration(remaining_seconds), inline=True)
